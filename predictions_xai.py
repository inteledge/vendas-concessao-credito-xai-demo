...
def plot_importances(best_pipeline, df, entry):
	# predictions
	df = pd.concat([df, entry])
	pred = best_pipeline.predict(df).values[-1]
	pred = 'Aprovado' if pred == 1 else 'Reprovado'
	pred_proba = best_pipeline.predict_proba(df).values[-1]
	...
	df_plot = explain_predictions(pipeline=best_pipeline, input_features=df.reset_index(drop=True),
							y=None, top_k_features=len(df.columns), indices_to_explain=[len(df)-1],
	...
	for col in df_plot['feature_names'].values:
		col_names.append(f'{col}<br><em>({entry[col].values[0]})</em>')
...
best_pipeline, df_sample, expected_value = get_pickles()
...
with col2:
	...
	fig_xai, predicao = plot_importances(best_pipeline, df_sample, df_inference)
